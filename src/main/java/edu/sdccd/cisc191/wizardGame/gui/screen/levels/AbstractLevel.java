...
public abstract class AbstractLevel extends KeyInput {
        ...
        for (int xx = 0; xx < w; xx++) {
            for (int yy = 0; yy < h; yy++) {
                ...
                if (red == 255 && green == 0 && blue == 255)
                    handler.addObject(new Knight(xx * 32, yy * 32, ID.Knight, handler, game, this, cs));       // pure magenta
    ...
    public void render(Graphics2D g) {
        ...
        if(this.getLives() <= 0) {
            // Enter game over game state.
            if (!this.game.isGameOver()) {
                this.game.pauseGame();
                this.game.getFrame().showGameOver(false);
                this.game.setGameOver(true); // Prevents from re-entering endGame
            }
        }
    } // end render
